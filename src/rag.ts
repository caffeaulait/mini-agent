...
const RAG_FILE = resolve('.miniagent/rag/index.json');
...
export async function loadRag(): Promise<void> {
  try {
    const value = JSON.parse(await readFile(RAG_FILE, 'utf8')) as {
      docs?: unknown;
    };
...
export async function addToRag(source: string): Promise<string> {
  ...
  if (
    ...
    return '已取消采集';
  if (docs.some((d) => d.source === target))
    return '已在知识库中，跳过（重复采集）';
  ...
  if (text.length > MAX_DOC_CHARS)
    return `内容过大（${text.length} 字符，上限 2MB），未入库`;
...
export function ragStats(): string {
  if (docs.length === 0)
    return '知识库还是空的（用 /rag add 采集，或让模型调用 rag_add）';
  const blocks = docs.reduce((n, d) => n + d.chunks.length, 0);
  const chars = docs.reduce(
    (n, d) => n + d.chunks.reduce((m, c) => m + c.text.length, 0),
    0,
  );
  return `知识库共 ${docs.length} 个文档 / ${blocks} 块 / ${chars} 字符：\n${docs
    .map(
      (d, i) =>
        `${i + 1}. 《${d.title}》（${d.chunks.length} 块，${d.source}）`,
    )
    .join('\n')}`;
}
  ...
  const tools: Tool[] = [
    {
      ...
      parameters: {
        ...
        properties: {
          source: {
            type: 'string',
            description: '网页完整 URL（http/https）或仓库内文件路径',
          },
        },
        required: ['source'],
      ...
      parameters: {
        ...
        properties: {
          query: {
            type: 'string',
            description:
              '要检索的说法，直接写自然语言，如「这套教程 Day 5 做了什么」',
          },
        },
        required: ['query'],
        additionalProperties: false,
