  ...
  const tools: Tool[] = [
    {
      name: 'memory_write',
      description:
        '把值得跨会话保留的用户偏好、项目事实或重要决定写入长期记忆。不要记录临时任务进度或可随时从文件读到的内容。',
      parameters: {
        type: 'object',
        properties: {
          content: {
            type: 'string',
            description: '一条独立、简洁、脱离当前对话也能理解的事实',
          },
        },
        required: ['content'],
    ...
    },
    {
      name: 'memory_search',
      description:
        '按关键词搜索长期记忆。需要回忆用户偏好、项目事实或以前的决定时调用；query 使用一个或多个空格分隔的关键词。',
      parameters: {
        type: 'object',
        properties: {
          query: {
            type: 'string',
            description: '搜索关键词，多个词用空格分隔，如“博客 人称”',
          },
        },
        required: ['query'],
        additionalProperties: false,
