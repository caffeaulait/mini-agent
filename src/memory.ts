...
const MEMORY_FILE = resolve('.miniagent/memory.json');
...
function formatHits(hits: Chunk[]): string {
  return hits
    .map(
      (c, i) =>
        `${i + 1}. （条目 #${c.entry + 1}，第 ${c.index + 1} 段）${c.text.trim()}`,
    )
    .join('\n');
}
  ...
  const tools: Tool[] = [
    {
      ...
      parameters: {
        ...
        properties: {
          content: {
            type: 'string',
            description: '一条脱离当前对话也能独立理解的事实，可以是一段长文',
          },
        },
        required: ['content'],
      ...
      run: async (args) => {
        ...
        changed();
        return entry.length < content.length
          ? `已记住（过长已截短）：${entry}`
          : `已记住：${entry}`;
      },
    },
    {
      name: 'memory_search',
      description:
        '按 BM25 打分检索长期记忆，返回命中的记忆块（精确到段）。需要回忆用户偏好、项目事实或以前的决定时调用；query 使用一个或多个空格分隔的关键词，如“博客 人称”。',
      parameters: {
        type: 'object',
        properties: {
          query: {
            type: 'string',
            description: '搜索关键词，多个词用空格分隔，关键词越聚焦命中越准',
          },
        },
        required: ['query'],
        additionalProperties: false,
