  ...
  const tools: Tool[] = [
    {
      name: 'todo_write',
      description:
        '写入完整 TODO 列表，用于规划和更新任务进度。开始一项时标为 in_progress，完成后标为 completed；同一时间只保留一个 in_progress。',
      parameters: {
        type: 'object',
        properties: {
          todos: {
      ...
      run: (args) => {
        const next = Array.isArray(args.todos)
    ...
    },
    {
      name: 'delegate_task',
      description:
        '把一个边界清楚的子任务交给隔离上下文的子 Agent，等待它完成后返回结果。适合分析、设计和审查；多个子任务应逐个调用。',
      parameters: {
        type: 'object',
        properties: {
