...
/** .miniagent/mcp.json 里一个 server 的启动配置：stdio 模式下，它就是一个本地子进程。 */
...
const PROTOCOL_VERSION = '2026-08-18';
...
class McpClient {
  ...
  async start(): Promise<McpToolInfo[]> {
    ...
    this.proc.on('error', (err) =>
      this.markDead(`server 无法启动：${err.message}`),
    );
    this.proc.on('exit', (code) =>
      this.markDead(`server 进程已退出（code ${code}）`),
    );
  ...
  private request(method: string, params: unknown): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        reject(
          new Error(
            `请求 ${method} 超时（${REQUEST_TIMEOUT_MS / 1000}s 无回应）`,
          ),
        );
      }, REQUEST_TIMEOUT_MS);
  ...
  for (const [name, cfg] of Object.entries(config.mcpServers ?? {})) {
    ...
    try {
      ...
      for (const info of tools) {
        ...
        ensureToolPolicy(tool.name, 'ask'); // 外部进程提供的工具默认 ask，miniAgent.json 里可改 allow / deny
        registerTool(tool);
