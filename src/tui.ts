...
class Panel implements Component {
  ...
  render(width: number): string[] {
    return this.getLines().map((line) =>
      paint('sys', `│ ${truncateToWidth(line, Math.max(1, width - 2))}`),
    );
  }
...
class InputRow implements Component, Focusable {
  ...
  render(width: number): string[] {
    ...
    const value = sliceByColumn(
      this.input.render(inputWidth + 2)[0] ?? '',
      2,
      inputWidth,
      true,
    );
...
export class TUI {
  ...
  private screen = new TuiAltScreen(this.terminal, true, undefined, {
    mouse: true,
  });
  ...
  constructor(
    ...
    const body = new HStack(
      [
        {
          component: new ScrollView(this.log, {
            follow: 'end',
            primary: true,
            scrollbar: 'auto',
          }),
  ...
  ready(): void {
    if (!this.busy && this.confirmQuestion === null)
      this.screen.setFocus(this.input);
  }
  ...
  private render(): void {
    this.log.setText(
      this.lines.map((line) => paint(line.color, line.text)).join('\n'),
    );
    this.screen.requestRender();
  }
}
