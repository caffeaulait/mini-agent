      ...
      switch (command) {
        ...
        case '/sessions': {
          ...
          const lines = list.map(
            (item) =>
              `${item.current ? '*' : ' '} ${item.id}（${item.count} 条消息）`,
          );
          out('sys', `会话：\n${lines.join('\n')}`, true);
          break;
        }
...
rl.on('close', async () => {
  try {
    ...
    if (generatedId) {
      out(
        'sys',
        `（会话 ID：${generatedId}，重启后可用 /load 再用 /open ${generatedId} 打开）`,
        true,
      );
    }
  } catch (e) {
...
out(
  'sys',
  `Mini Agent Day 6 —— 多会话与持久化（模型：${config.model}，当前会话：${sessions.currentId()}）`,
  true,
);
rl.prompt();
