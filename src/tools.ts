...
const BUILTIN_TOOLS: Tool[] = [
  {
    ...
    description: '获取当前本地时间（Asia/Singapore）。',
  ...
  },
  {
    name: 'ls',
    description:
      '列出目录下的条目（只读）。默认当前目录，目录元素带 / 后缀，目录在前。',
    parameters: {
      type: 'object',
      properties: {
        path: {
  ...
  },
  {
    name: 'read',
    description:
      '读取文本文件内容（只读）。文件过大时自动截断，可用 offset 从指定字符偏移处分段续读。',
    parameters: {
      type: 'object',
      properties: {
        ...
        offset: {
          type: 'number',
          description: '从第 offset 个字符开始读，默认 0；用于分段读大文件。',
        },
      },
      required: ['path'],
      additionalProperties: false,
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        pattern: {
          type: 'string',
          description: 'glob 模式，如 "day4/*.ts" 或 "**/tools.ts"。',
        },
      },
      required: ['pattern'],
      additionalProperties: false,
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        ...
        content: {
          type: 'string',
          description: '文件的新完整内容（原内容将被整体替换）。',
        },
      },
    ...
    parameters: {
      ...
      properties: {
        ...
        hunks: {
          type: 'array',
          description:
            '修改片段列表：每个片段把文件中唯一匹配的 old 替换为 new。',
          items: {
            type: 'object',
            properties: {
              old: {
...
function simpleDiff(oldText: string, newText: string): string {
  ...
  for (
    let i = Math.max(0, b.length - q);
    i < Math.min(b.length, b.length - q + ctx);
    i++
  )
    out.push(' ' + b[i]);
